...
# Не подставляйте чужие ID серверов: без .env бот не привяжется к случайному форуму.
_UNSET_SNOWFLAKE = 0


def _env_channel_id_required(name: str) -> int:
    """ID канала/ветки из .env; 0 = не задано (не использовать старые захардкоженные форумы)."""
    return _env_channel_id(name, _UNSET_SNOWFLAKE)
...
def _env_int_clamped(name: str, default: int, *, min_v: int, max_v: int) -> int:
    raw = (os.environ.get(name) or "").strip()
    if not raw:
        return default
    try:
...
# ID каналов-форумов задаются в .env (обязательно на проде). Значение 0 = не настроено.
FORUM_CHANNEL_ID = _env_channel_id_required("DISCORD_ANIME_FORUM_CHANNEL_ID")
LIST_FORUM_CHANNEL_ID = _env_channel_id_required("DISCORD_LIST_FORUM_CHANNEL_ID")
BOT_INFO_THREAD_ID = _env_channel_id_required("DISCORD_BOT_INFO_THREAD_ID")
...
DISCORD_FORUM_MERGE_DELAY_SEC = _env_float("DISCORD_FORUM_MERGE_DELAY_SEC", 0.18, minimum=0.05)
MAL_FETCH_PAGE_DELAY_SEC = _env_float("MAL_FETCH_PAGE_DELAY_SEC", 0.15, minimum=0.05)
PERSONAL_META_FETCH_CONCURRENCY = _env_int_clamped(
    "PERSONAL_META_FETCH_CONCURRENCY", 6, min_v=1, max_v=12
)
BOT_INFO_MESSAGE_DELAY_SEC = _env_float("BOT_INFO_MESSAGE_DELAY_SEC", 0.22, minimum=0.08)
BOT_INFO_DELETE_DELAY_SEC = _env_float("BOT_INFO_DELETE_DELAY_SEC", 0.12, minimum=0.05)
...
async def mal_fetch_full_list(
    ...
    while True:
        ...
        if len(chunk) < 50:
            break
        await asyncio.sleep(MAL_FETCH_PAGE_DELAY_SEC)
...
async def read_state_copy() -> dict[str, Any]:
    async with _state_lock:
        return copy.deepcopy(_load_state())
...
async def _drop_anime_topic_key(key: str) -> None:
    """Удаляет запись anime_topics (например, ветка в старом форуме после смены DISCORD_ANIME_FORUM_CHANNEL_ID)."""
    async with _state_lock:
        data = _load_state()
        topics = data.setdefault("anime_topics", {})
        topics.pop(key, None)
...
async def merge_adder_into_existing_topic(
    ...
    if FORUM_CHANNEL_ID > 0 and int(thread.parent_id) != FORUM_CHANNEL_ID:
        logger.warning(
            "anime_topics[%r]: ветка %s в форуме %s, а основной форум в .env — %s. "
            "Запись удалена; будет создана новая тема в нужном форуме.",
            key,
            thread_id,
            thread.parent_id,
            FORUM_CHANNEL_ID,
        )
        await _drop_anime_topic_key(key)
        return None, ""
...
def _build_bot_info_embed_groups() -> list[list[discord.Embed]]:
    """Сообщения для справочной ветки (по одному embed на сообщение — проще редактировать)."""
    tip = discord.Embed(
        title="💡 Полезное",
        description=(
            "**Эта ветка** — единая точка входа: команды и подсказки ниже.\n\n"
            "**Как обновляется:** при **запуске бота** и командой **`/admin refresh_help_thread`** "
            "(нужны права администратора или роль из `DISCORD_ADMIN_ROLE_ID`).\n\n"
            "· **Основной форум аниме** — общие темы. Под **первым** сообщением темы: "
            ...
            "между перезапусками (см. `.env.example`).\n\n"
            "В обычном чате можно писать: **`!aa запрос`**, **`!animeadd запрос`** — то же, что slash **`/forum_add`**."
        ),
        color=EMBED_COLOR,
    )
    tip.set_footer(text=f"ID ветки в .env: DISCORD_BOT_INFO_THREAD_ID · сейчас: {BOT_INFO_THREAD_ID}")

    forum = discord.Embed(
        title="📌 Команды: основной форум и добавление аниме",
        color=EMBED_COLOR,
    )
    forum_rows = [
        (
        ...
        (
            "`/rateanime`",
            "Поставить оценку **1–10** (вызывать **внутри ветки** основного форума).",
        ),
        (
            "`/update_topics`",
            "**Админы:** пройтись по веткам форума — реакции, боковые панели, при необходимости обновить карточку Yummy.",
        ),
        (
            "`/checkduplicates`",
            "Найти дубликаты тем на одно аниме и убрать лишние.",
        ),
    ]
    for name, val in forum_rows:
        forum.add_field(name=name, value=val, inline=False)

    mylist = discord.Embed(
        title="📋 Команды: личный список",
        color=EMBED_COLOR,
    )
    mylist_rows = [
        (
    ...
    for name, val in mylist_rows:
        mylist.add_field(name=name, value=val, inline=False)
    mylist.set_footer(text="Личную тему бот создаёт в форуме из DISCORD_LIST_FORUM_CHANNEL_ID.")

    yummy = discord.Embed(
        title="🍱 Команды: YummyAnime",
        color=EMBED_COLOR,
    )
    yummy_rows = [
        (
            "`/yummy_link` · `/yummy_token`",
    ...
    for name, val in yummy_rows:
        yummy.add_field(name=name, value=val, inline=False)

    mal = discord.Embed(
        title="🎌 Команды: MyAnimeList",
        color=EMBED_COLOR,
    )
    mal_rows = [
    ...
    for name, val in mal_rows:
        mal.add_field(name=name, value=val, inline=False)

    admin = discord.Embed(
        title="🛠 Админы и сервис",
        color=EMBED_COLOR,
    )
    admin_rows = [
        (
        ...
        (
            "Переменные среды",
            "Каналы форумов: `DISCORD_ANIME_FORUM_CHANNEL_ID`, `DISCORD_LIST_FORUM_CHANNEL_ID`, "
            "`DISCORD_BOT_INFO_THREAD_ID`. Yummy API: `YUMMY_APPLICATION_TOKEN`. "
            "Опционально: `MONGODB_URI`, `MYLIST_ADMIN_SYNC_REPAIR_MAIN_FORUM`.",
        ),
    ]
    for name, val in admin_rows:
        admin.add_field(name=name, value=val, inline=False)
    admin.set_footer(
        text="Панели «Оценка» и «Рекомендация» под первым сообщением новых тем создаёт бот автоматически."
    )

    return [[tip], [forum], [mylist], [yummy], [mal], [admin]]


async def refresh_bot_help_thread(client: discord.Client) -> bool:
    """
    Публикует или правит embed-сообщения в справочной ветке (BOT_INFO_THREAD_ID).
    ID сообщений хранятся в meta.bot_info_message_ids.
    """
    me = client.user
    if me is None:
        return False
    if BOT_INFO_THREAD_ID <= 0:
        logger.warning("Справка: не задан DISCORD_BOT_INFO_THREAD_ID — пропуск обновления ветки.")
        return False
    ...
    if not isinstance(ch, discord.Thread):
        logger.warning(
            "Справка: ветка %s недоступна (проверьте DISCORD_BOT_INFO_THREAD_ID и права бота).",
            BOT_INFO_THREAD_ID,
        )
        return False
    if ch.archived:
        logger.warning(
            "Справка: ветка %s в архиве — разархивируйте, чтобы обновлять посты.",
            BOT_INFO_THREAD_ID,
        )
        return False

    groups = _build_bot_info_embed_groups()
    async with _state_lock:
        raw_ids = (_load_state().get("meta") or {}).get("bot_info_message_ids")

    old_ids: list[int] = []
    if isinstance(raw_ids, list):
        for x in raw_ids:
            try:
                old_ids.append(int(x))
            except (TypeError, ValueError):
                continue

    new_ids: list[int] = []
    for i, embeds in enumerate(groups):
        placed = False
        if i < len(old_ids):
            try:
                msg = await ch.fetch_message(old_ids[i])
        ...
        if not placed:
            ...
            new_ids.append(msg.id)
        await asyncio.sleep(BOT_INFO_MESSAGE_DELAY_SEC)

    for j in range(len(groups), len(old_ids)):
        try:
            msg = await ch.fetch_message(old_ids[j])
            if msg.author.id == me.id:
                await msg.delete()
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            pass
        await asyncio.sleep(BOT_INFO_DELETE_DELAY_SEC)

    async with _state_lock:
        data = _load_state()
        data.setdefault("meta", {})["bot_info_message_ids"] = new_ids
        _write_state(data)

    logger.info("Справочная ветка обновлена: %s сообщений.", len(new_ids))
    return True


async def ensure_bot_info_thread(client: discord.Client) -> None:
    """Фиксированная ветка справки: BOT_INFO_THREAD_ID; meta и при старте — refresh постов."""
    if BOT_INFO_THREAD_ID <= 0:
        logger.warning(
            "Справка: задайте **DISCORD_BOT_INFO_THREAD_ID** в .env (ID ветки с подсказками)."
        )
        return
    async with _state_lock:
        data = _load_state()
...
async def resolve_forum_channel(client: discord.Client) -> discord.ForumChannel | None:
    if FORUM_CHANNEL_ID <= 0:
        logger.warning(
            "Не задан **DISCORD_ANIME_FORUM_CHANNEL_ID** в .env — добавление в основной форум невозможно."
        )
        return None
...
async def resolve_list_forum_channel(client: discord.Client) -> discord.ForumChannel | None:
    if LIST_FORUM_CHANNEL_ID <= 0:
        logger.warning(
            "Не задан **DISCORD_LIST_FORUM_CHANNEL_ID** в .env — личные списки недоступны."
        )
        return None
...
def _ordered_keys_for_personal_display(
    state: dict[str, Any], pl: dict[str, Any]
) -> list[str]:
    """Один ключ на одну ветку основного форума — убирает дубли карточек (разные slug на ту же тему)."""
    base = _ordered_keys_for_personal(pl)
    topics = state.get("anime_topics", {})
    ...
    for k in base:
        ent = topics.get(k)
        tid = int(ent.get("thread_id") or 0) if isinstance(ent, dict) else 0
        if tid > 0:
            if tid in seen_tid:
                continue
            seen_tid.add(tid)
...
def _other_members_with_same_anime(
    ...
    """Имена участников, у которых в личном списке то же аниме (тот же ключ или та же ветка форума)."""
    topics = state.get("anime_topics", {})
    ent = topics.get(anime_key) if isinstance(topics, dict) else None
    want_tid = int(ent.get("thread_id") or 0) if isinstance(ent, dict) else 0
    names: list[str] = []
    for uid_s, pl in (state.get("personal_lists") or {}).items():
        try:
            ...
            continue
        if uid == owner_id or not isinstance(pl, dict):
            continue
        found = False
        for k in _ordered_keys_for_personal(pl):
            if k == anime_key:
                ...
                break
            if want_tid:
                e2 = topics.get(k) if isinstance(topics, dict) else None
                t2 = int(e2.get("thread_id") or 0) if isinstance(e2, dict) else 0
                if t2 == want_tid and want_tid:
                    found = True
                    break
        if found:
            mem = guild.get_member(uid)
            names.append(mem.display_name if mem else str(uid))
    if not names:
        return ""
...
class PersonalTopicHubView(discord.ui.View):
    ...
    async def hub_export(
        ...
        await interaction.response.defer(ephemeral=True, thinking=True)
        state = await read_state_copy()
        keys = _ordered_keys_for_personal_display(state, pl)
        lines: list[str] = []
        ...
        bio.seek(0)
        fname = f"anime_list_{owner_id}.txt"
        await interaction.followup.send(
            content=f"Полный список (**{len(keys)}** позиций) во вложении.",
...
def _build_personal_anime_card_embed(
    ...
    *,
    ...
    if others_line is None and guild is not None:
        others_line = _other_members_with_same_anime(state, guild, owner_id, anime_key)
    if others_line:
        others_line = _truncate(others_line, EMBED_FIELD_LIMIT)

    if compact:
        extra = f" · 👥 {others_line}" if others_line else ""
        embed.description = _truncate(
            f"📊 {src_label}: {global_line} · Ваша оценка: {user_line}{extra}",
            EMBED_DESC_LIMIT,
        )
    else:
...
async def rebuild_personal_list_display(
    ...
    *,
    ...
    state = await read_state_copy()
    pl = (state.get("personal_lists") or {}).get(uid_s, pl)
    if not isinstance(pl, dict):
        return
    ...
    keys = _ordered_keys_for_personal_display(state, pl)
    sem = asyncio.Semaphore(PERSONAL_META_FETCH_CONCURRENCY)

    async def _bounded_meta(k: str) -> dict[str, Any]:
        async with sem:
            return await _fetch_personal_card_meta(session, state, k)

    meta_by_key: list[dict[str, Any]] = (
        list(await asyncio.gather(*(_bounded_meta(k) for k in keys))) if keys else []
    )

    new_map: dict[str, int] = {}
    for i, (anime_key, meta) in enumerate(zip(keys, meta_by_key), start=1):
        ...
        await asyncio.sleep(DISCORD_LIST_CARD_DELAY_SEC)
    ...
    *,
    ...
    async with _personal_list_thread_lock(uid):
        ...
        intro = (
            ...
            "Название и текст: `/mylist_edit` · топ-5: `/mylist_top` · синк с форумом: `/mylist_admin_sync` (админ).\n"
        ...
        # Панель создаётся только в rebuild_personal_list_display — иначе дублируется с первым refresh.
        if starter is not None:
            await _save_personal_thread_meta(
                uid,
                thread_id=thread.id,
...
def list_personal_anime_pairs(
    ...
    if not isinstance(pl, dict):
        return []
    out: list[tuple[str, str]] = []
    for k in _ordered_keys_for_personal_display(state, pl):
        t = _title_for_list_key(state, k)
...
async def run_yummy_list_import_for_member(
    ...
    *,
    ...
    if not forum:
        empty["error"] = (
            "Основной форум не настроен: задайте **DISCORD_ANIME_FORUM_CHANNEL_ID** в `.env` "
            "или проверьте права бота."
        )
        return empty
        ...
        if info:
            ...
            if mst == "merged":
                ...
                if merge_ops >= merge_limit:
                    ...
                    break
                await asyncio.sleep(DISCORD_FORUM_MERGE_DELAY_SEC)
                continue
            if mst == "already":
                await mark_yummy_imported(uid, aid)
                imported_ids.add(aid)
...
async def yummy_background_poll_loop() -> None:
    ...
    while not bot.is_closed():
        ...
        if app and bot.session:
            ...
            if g:
                _yf = (os.environ.get("YUMMY_POLL_LIST_FILTER") or "completed").strip().lower()
                if _yf not in (
                    "all",
                    "watching",
                    "plan_to_watch",
...
class YummyDiscordLoginModal(discord.ui.Modal, title="Вход YummyAnime"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.followup.send(
            f"YummyAnime привязан (**{nick or yid_i}**). Импорт: `/yummy_sync` или **Yummy «Просмотрено»**.\n"
...
class YummyLinkVerifyView(discord.ui.View):
    ...
    async def verify(
        ...
        await interaction.response.send_message(
            f"YummyAnime привязан (**{nick or yuid}**). Импорт: `/yummy_sync` или кнопка **Yummy «Просмотрено»** в личной теме.\n"
...
async def run_animeadd_for_user(guild: discord.Guild, user_id: int, query: str) -> str:
    ...
    if not ch:
        return (
            "Канал **основного** форума не найден. Укажите **DISCORD_ANIME_FORUM_CHANNEL_ID** в `.env` "
            "(ID форум-канала с аниме) и перезапустите бота; проверьте права бота на создание тем."
        )
...
async def on_ready() -> None:
    ...
    try:
        await refresh_bot_help_thread(bot)
    except Exception:
        logger.exception("Справочная ветка: не удалось обновить посты с командами")
...
async def _yummy_token_impl(interaction: discord.Interaction, bearer_token: str) -> None:
    ...
    await interaction.followup.send(
        f"YummyAnime привязан (**{nick or yid_i}**). Импорт: `/yummy_sync` или кнопка **Yummy «Просмотрено»** в личной теме.\n"
...
@admin_cmd_group.command(
    name="refresh_help_thread",
    description="Обновить справочную ветку: команды и советы (DISCORD_BOT_INFO_THREAD_ID)",
)
async def admin_refresh_help_thread(interaction: discord.Interaction) -> None:
    ok, err = _admin_member_ok(interaction)
    if not ok:
        await interaction.response.send_message(err, ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True, thinking=True)
    try:
        ok2 = await refresh_bot_help_thread(bot)
    except Exception:
        logger.exception("admin refresh_help_thread")
        await interaction.followup.send(
            "Ошибка при обновлении справочной ветки (см. логи бота).", ephemeral=True
        )
        return
    if ok2:
        await interaction.followup.send(
            f"Справка обновлена в ветке <#{BOT_INFO_THREAD_ID}>.", ephemeral=True
        )
    else:
        await interaction.followup.send(
            "Не удалось обновить справку: ветка не найдена, нет прав или она в архиве.",
            ephemeral=True,
        )
...
class AdminPanelView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            ...
            discord.SelectOption(
                label="Справочная ветка",
                value="refresh_help_thread",
                description="Обновить посты с командами (BOT_INFO_THREAD_ID)",
            ),
        ],
    )
    async def admin_menu(
        ...
        if choice == "repair_topics":
            ...
            return
        if choice == "refresh_help_thread":
            try:
                ok2 = await refresh_bot_help_thread(bot)
            except Exception:
                logger.exception("adminpanel refresh_help_thread")
                await interaction.followup.send(
                    "Ошибка при обновлении справочной ветки.", ephemeral=True
                )
                return
            if ok2:
                await interaction.followup.send(
                    f"Справка обновлена: <#{BOT_INFO_THREAD_ID}>.", ephemeral=True
                )
            else:
                await interaction.followup.send(
                    "Не удалось: ветка не найдена, нет прав или она в архиве.", ephemeral=True
                )
