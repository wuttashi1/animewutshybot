...
def setup(bot_instance: discord.Client) -> None:
    ...
    # Canonical groups are the default. Old aliases are an explicit migration option.
    if os.environ.get("DISCORD_LEGACY_COMMANDS", "0").lower() not in ("1", "true", "yes"):
        return
    ...
    async def legacy_mylist(
        ...
        await list_show.callback(interaction, member)
    ...
    async def legacy_yummybind(
        ...
        await yummy_bind.callback(interaction, bearer_token)
    ...
    async def legacy_yummyunbind(interaction: discord.Interaction) -> None:
        await yummy_unbind.callback(interaction)
    ...
    async def legacy_syncyummy(
        ...
        await yummy_sync.callback(interaction, yummy_list, max_topics)
    ...
    async def legacy_malbind(interaction: discord.Interaction, list_url: str) -> None:
        await mal_bind.callback(interaction, list_url)
    ...
    async def legacy_connectmyanimelist(
        ...
        await mal_import.callback(interaction, mal_status, max_topics=max_topics)
