...
Повторная настройка использует уже привязанные форумы. Если они недоступны, проверьте `/bot health`: бот не создаёт замену поверх существующей конфигурации. Старые кнопки оценок и рекомендаций восстанавливаются по сохранённым ID без редактирования сообщений, включая архивные темы. При запросе обновления списка или оценки бот открывает архивную тему; блокировку модератора он не снимает. Для команд владельца используется только `DISCORD_BOT_OWNER_ID`, совпадения имени недостаточно.
