...
async def _yani_request(
    ...
    *,
    ...
    if method != "GET":
        raise ValueError("Only idempotent reads may use the retry client")
    return await get_json(session, f"{YANI_BASE}{path}",
...
async def yani_login_password(
    ...
    for body in payloads:
        try:
            async with session.post(f"{YANI_BASE}/profile/login", headers=headers, json=body, timeout=aiohttp.ClientTimeout(total=20), allow_redirects=False) as resp:
                status = resp.status
                saw_http.add(status)
...
async def yani_fetch_lists_with_token_refresh(
    ...
    if st == 401:
        ...
        if not new_tok:
            return None, None, "Сессия YummyAnime недействительна — выполните `/yummy bind` снова."
