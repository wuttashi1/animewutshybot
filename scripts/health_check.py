...
async def check_yummy_api() -> list[str]:
    ...
    async with aiohttp.ClientSession(headers={"User-Agent": USER_AGENT}) as session:
        ...
        if yummy_app:
            ...
            headers = yummy_api.build_yani_headers(yummy_app, None, USER_AGENT)
            async with session.post(
                "https://api.yani.tv/profile/login",
                headers=headers,
                json={"login": "healthcheck", "password": "healthcheck"},
            ) as resp:
                lines.append(f"POST /profile/login → HTTP {resp.status}")
                if resp.status == 404:
                    lines.append("  FAIL: login endpoint not found")
                elif resp.status in (401, 403, 422):
                    lines.append("  OK: endpoint reachable (expected auth error)")
                elif resp.status in (200, 201):
                    lines.append("  OK: endpoint reachable (returned success)")
                else:
                    lines.append(f"  WARN: unexpected status {resp.status}")
        else:
            lines.append("POST /profile/login: SKIP (no YUMMY_APPLICATION_TOKEN)")
    return lines
